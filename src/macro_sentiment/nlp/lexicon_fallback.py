...
def score_text(text: str) -> dict:
    """Metin için {polarity[-1,1], intensity[0,100], confidence, fear, greed} döndürür."""
    words = _WORD_RE.findall(text.lower())
    if not words:
        return {"polarity": 0.0, "intensity": 0.0, "confidence": 0.3, "fear": 0.0, "greed": 0.0}
    ...
    return {
        ...
        "fear": round(min(1.0, fear / n * 20), 3),
        "greed": round(min(1.0, greed / n * 20), 3),
    }
